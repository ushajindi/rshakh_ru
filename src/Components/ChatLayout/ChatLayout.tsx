...
const ChatLayout = (props: any) => {
    ...
    useEffect(() => {
        scrollToBottom();
    }, [rootStore.ChatStore.Chats]);

    const scrollToBottom = () => {
        if (chatLayoutRef.current) {
            chatLayoutRef.current.classList.add('scroll-animation');
            chatLayoutRef.current.scrollTop = chatLayoutRef.current.scrollHeight;
            // Удалите класс анимации после некоторого времени
            setTimeout(() => {
                if (chatLayoutRef.current){
                    chatLayoutRef.current.classList.remove('scroll-animation');
                }
    ...
    return (
        <div style={{ width: props.width }} className="chatlayout">
            ...
            <div style={{ display: props.chatname }} className="chatlayout__box">
                <div className="chatlayout__box__inner">
                    <div className="chatlayout__box__items">
                        <h2 className="chatlayout__box__text">
                            {rootStore.GeneralPage.activeNameChat ? rootStore.GeneralPage.activeNameChat : "Начнем общение?"}
                        </h2>
            ...
            <div ref={chatLayoutRef} className="chatlayout__inner">
                    ...
                    rootStore.ChatStore.Chats.map((el) => {
                        if (el?._id === rootStore.GeneralPage.ActiveChatId) {
                            return el?.messages.map((message) => {
                                return <Message scroll={scrollToBottom} messages={message} width={props.width} />;
                            });
                        }
                    })
                ) :
                ...
                {
                    !rootStore.GeneralPage.ActiveChatId&&<div className="chat__text">
                        Свайп влево чтобы начать общаться<br/>
                        Свайп вправо ваш профиль

                    </div>
                }
                <div  className="chatlayout__message__end"></div>
